...
public class aws_server {
	...
	public static void main(String[] args) {
		...
		try {
			network_manager_instance.initialize_network(2033);
		} catch (Exception e) {
		}
		...
		while (!terminated) {
			...
			if (command.compareTo("stop") == 0) {
				...
				for (user_data user : game_data.users) {
					System.out
							.println(user.id + ":: x = " + String.valueOf(user.x) + ", y = " + String.valueOf(user.y));
				}
	...
	private static void run_unit_test(aws_connector aws) {
		...
		if (success) {
			try {
				S3Object object = aws.s3.getObject(aws.bucketName,
						log_generator.log_prefix + "-1" + log_generator.log_suffix);
				...
				if (log_generator.log_buffer[0].compareTo(s3buffer[0]) == 0
						&& log_generator.log_buffer[1].compareTo(s3buffer[1]) == 0
						&& log_generator.log_buffer[2].compareTo(s3buffer[2]) == 0) {
					success = true;
				} else {
